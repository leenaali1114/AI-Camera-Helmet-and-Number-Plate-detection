    ...
    for i in range(len(boxes)):
        if i in indexes:
            ...
            if classIds[i]==0: #bike
                ...
                number_plate_roi = cv2.resize(number_plate_roi, (number_plate_roi.shape[1]*2, number_plate_roi.shape[0]*2))
                
                # Convert to grayscale
                gray = cv2.cvtColor(number_plate_roi, cv2.COLOR_BGR2GRAY)
                ...
                # Additional dilation to make the text thicker
                ...
                if len(plate_number) >= 4:  # Only show if we have a reasonable length plate number
                    print(f"Detected Number Plate: {plate_number}")
                    cv2.putText(img, plate_number, (x, y-50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
                    
                    # Optionally save the processed plate image for debugging
                    # cv2.imwrite(f'plate_{plate_number}.jpg', thresh)
